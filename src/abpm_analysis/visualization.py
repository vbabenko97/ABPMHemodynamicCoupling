...
class Figure2Generator(FigureGenerator):
    ...
    def generate(self, df: pd.DataFrame, res_df: pd.DataFrame) -> None:
        ...
        ax2.set_title(r"B: Signed residual bias ($\Delta Bias_{i,cog}$)", fontsize=16, fontweight='bold', pad=15)
        ax2.set_ylabel(r"Signed residual bias, $\Delta Bias$ (mmHg)", fontsize=14)
